            ...
            <div id="ptab-certs" class="profile-tab-content">
                ...
                <div class="border border-cbk rounded-2xl p-4 sm:p-6 flex flex-col md:flex-row items-center gap-5 sm:gap-6 bg-cbk-s2 relative overflow-hidden group">
                    ...
                    <div class="flex flex-col sm:flex-row gap-3 relative z-10 w-full md:w-auto cert-action-row">
                        ...
                        <button onclick="window.open('https://ciminds.in/verify?id=' + (state.certId || ''), '_blank')" class="bg-cbk-s2 border border-cbk-md hover:border-violet text-cbk px-4 py-2.5 rounded-xl font-bold text-sm transition-colors flex items-center justify-center shadow-sm">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg> Verify Certificate
                        </button>
...
async function downloadCertificate() {
    ...
    try {
        ...
        // ── Overlay verify URL — bottom left, small text ──────────────────
        if (state.certId) {
            const verifyUrl = 'https://ciminds.in/verify?id=' + state.certId;
            pdf.setFontSize(7);
            pdf.setTextColor(150, 150, 180);
            pdf.text('Verify: ' + verifyUrl, 297 * 0.0527, 210 * 0.965);
        }
