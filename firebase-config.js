...
async function getGoogleRedirectResult() {
  try {
    ...
    // TEMP DIAGNOSTIC: surface the real Firebase error code on screen so it
    // can be read off a phone without opening devtools. Remove this once
    // the redirect issue is confirmed fixed.
    window.__ciMindsLastAuthError = { code: err.code, message: err.message };
    return null;
  }
}
